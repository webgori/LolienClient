...
#pragma warning restore CS1591
        ...
        public Version GetCurrentLeagueVersion()
        {
            ...
            return new Version(FileVersionInfo.GetVersionInfo(Path.Combine(this.RiotDirectory.FullName, "LeagueClient.exe")).ProductVersion);

            /*DirectoryInfo releases = new DirectoryInfo(Path.Combine(this.RiotDirectory.FullName, "RADS", "projects", "league_client", "releases"));
            ...
            foreach (DirectoryInfo release in releases.GetDirectories("*.*.*.*", SearchOption.TopDirectoryOnly))
                return new Version(FileVersionInfo.GetVersionInfo(Path.Combine(release.FullName, "deploy", "LeagueClient.exe")).ProductVersion);
        ...
        {
            if (new FileInfo(Path.Combine(givenRiotDirectory, "LeagueClient.exe")).Exists)
                return new DirectoryInfo(givenRiotDirectory);

            if (new FileInfo(Path.Combine(League.DefaultRiotDirectory, "LeagueClient.exe")).Exists)
